...
# ---------------------------------------------------------
# Notebook 00: Config & Setup (VPC + Subnet + GCP APIs)
# ---------------------------------------------------------
nb0_cells = [
    markdown_cell("""# Module 00: GCP Environment & Custom VPC Subnet Setup
Welcome to the JAX Multi-Node Training on GKE Learning Plan!

### Learning Objectives:
1. Load and inspect central environment variables (`PROJECT_ID`, `REGION`, `ZONE`, `CLUSTER_NAME`, etc.).
2. Authenticate and configure `gcloud` CLI.
3. Provision custom VPC Network and Subnet with IP-Aliasing (`pods-range`, `services-range`).
4. Enable required Google Cloud APIs (`container.googleapis.com`, `artifactregistry.googleapis.com`, `cloudbuild.googleapis.com`).
"""),
...
config.print_config()
"""),
    markdown_cell("## 1. Load GCP Environment Variables"),
    code_cell("""import os
...
print(f"Network / Subnet: {NETWORK_NAME} / {SUBNET_NAME}")
"""),
    markdown_cell("## 2. Configure `gcloud` CLI & Enable APIs"),
    code_cell("""!gcloud config set project {PROJECT_ID}
!gcloud config set compute/region {REGION}
!gcloud config set compute/zone {ZONE}
"""),
    code_cell("""print("Enabling required GCP Cloud APIs...")
...
"""),
    markdown_cell("## 3. Provision Custom VPC & Subnet with IP Aliasing"),
...
"""),
    markdown_cell("## 4. Verify GCP Credentials & VPC Status"),
...
# ---------------------------------------------------------
nb3_cells = [
    markdown_cell("""# Module 03: Multi-Node JAX CPU Training & 10x Scale-Up
